...
from .serializers import PartExchangeRequestSerializer
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:32px 40px 8px;text-align:center;">
              <h2 style="margin:0;color:#ac1c7a;font-size:22px;">New Part Exchange Valuation Request</h2>
              <p style="margin:8px 0 0;color:#888;font-size:14px;">A customer wants a valuation for their vehicle.</p>
            </td>
          </tr>
            ...
            <td style="padding:24px 40px;">
              <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
                ...
                <tr>
                  ...
                  <td style="padding:12px 16px;color:#222;font-size:14px;">{make} {model} ({year})</td>
                </tr>
                <tr style="background:#f9f9f9;">
                  <td style="padding:12px 16px;font-weight:bold;color:#555;font-size:14px;">Mileage</td>
                  <td style="padding:12px 16px;color:#222;font-size:14px;">{mileage}</td>
                </tr>
              </table>
            </td>
          </tr>
...
@extend_schema(
    tags=['Part Exchange'],
    summary='Submit a part exchange / vehicle valuation request',
    request=PartExchangeRequestSerializer,
    responses={
        201: inline_serializer(
            name='PartExchangeRequestSuccess',
            fields={
...
class PartExchangeRequestView(APIView):
    permission_classes = [AllowAny]

    def post(self, request):
        serializer = PartExchangeRequestSerializer(data=request.data)
        ...
        return Response(
            {"success": True, "message": "Thanks! Our team will contact you with a valuation shortly."},
    ...
    def _send_admin_email(self, submission):
        ...
        try:
            ...
            resend.Emails.send({
                ...
                "subject": f"[Part Exchange] {submission.make} {submission.model} ({submission.year})",
                "html": html_body,
            })
            logger.info("Admin notification sent for part exchange request %s", submission.pk)
        except Exception as exc:
            logger.error("Admin email failed for part exchange request %s: %s", submission.pk, exc, exc_info=True)
