...
def to_browser_safe_image(uploaded_file):
    ...
    if is_heic and not HEIF_SUPPORTED:
        raise ValueError(
            f"Cannot process '{name}': HEIC/HEIF support is unavailable on this "
            "environment (pillow_heif failed to load). Please convert this photo "
            "to JPEG/PNG before uploading, or fix the pillow_heif install."
        )
